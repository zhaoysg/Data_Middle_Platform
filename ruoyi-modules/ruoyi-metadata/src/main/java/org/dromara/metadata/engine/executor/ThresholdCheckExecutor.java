 ...
 * 规则类型: THRESHOLD
 * <p>
 * 检查目标列的值是否在指定的阈值范围内（min/max）。
 */
@Slf4j
public class ThresholdCheckExecutor extends AbstractRuleExecutor {
    ...
    public void execute(DqcRuleDef rule, DqcExecutionDetail detail, DataSourceAdapter adapter) {
        ...
        try {
            ...
            log.error("THRESHOLD执行失败: ruleId={}, error={}", rule.getId(), e.getMessage());
            detail.setElapsedMs(System.currentTimeMillis() - start);
